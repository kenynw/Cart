...
class ControllerCatalogCategory extends Controller {
	private $route = 'catalog/category';
	...
	protected function getList() {
		...
		$results = $this->model_catalog_category->getCategories_ ( $data );
		foreach ( $results as $result ) {
			...
			$this->data ['categories'] [] = array ( 'category_id' => $result ['category_id'], 'name' => $result ['name'], 'sort_order' => $result ['sort_order'], 'selected' => isset ( $this->request->post ['selected'] ) && in_array ( $result ['category_id'], $this->request->post ['selected'] ), 'action' => $action );
		}
		...
		$this->initNotice ();
		$this->initTpl ( 'catalog/category_list' );
	...
	protected function getForm() {
		...
		if (isset ( $this->request->get ['category_id'] ) && ($this->request->server ['REQUEST_METHOD'] != 'POST')) {
			$category_info = $this->model_catalog_category->getCategory_ ( $this->request->get ['category_id'] );
		}
		...
		if (isset ( $this->request->post ['category_filter'] )) {
			...
			$filters = $this->model_catalog_category->getCategoryFilters_ ( $this->request->get ['category_id'] );
		} else {
			$filters = array ();
		}
		...
		foreach ( $filters as $filter_id ) {
			...
			if ($filter_info) {
				$this->data ['category_filters'] [] = array ( 'filter_id' => $filter_info ['filter_id'], 'name' => $filter_info ['group'] . ' &gt; ' . $filter_info ['name'] );
			}
		}
		...
		$this->initNotice ();
		$this->initTpl ( 'catalog/category_form' );
		$this->children = array ( 'common/header', 'common/footer' );
		$this->response->setOutput ( $this->render () );
	}
