...
class ControllerOpenbayOpenbay extends Controller {
	...
	public function index() {
		...
		$this->initTpl('openbay/ebay_overview');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function settings() {
		...
		$this->initNotice();
		$this->initTpl('openbay/ebay_settings');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function getCategories() {
		...
		$json = $this->model_openbay_ebay->getCategory_($this->request->get['parent']);
	...
	public function viewSellerSummary() {
		...
		$this->initTpl('openbay/ebay_summary');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function setProductStock() {
		...
		$product = $this->model_catalog_product->getProduct_($this->request->get['product_id']);
	...
	public function viewSubscription() {
		...
		$this->initTpl('openbay/ebay_subscription');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function viewItemImport() {
		...
		$this->initTpl('openbay/ebay_item_import');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function viewOrderImport() {
		...
		$this->initTpl('openbay/ebay_order_import');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function viewSync() {
		...
		$this->initTpl('openbay/ebay_syncronise');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function viewItemLinks() {
		...
		$this->initTpl('openbay/ebay_item_link');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function edit() {
		if ($this->checkConfig() == true) {
			if (!empty($this->request->get['product_id'])) {
				...
				$this->initTpl('openbay/ebay_edit');
	...
	public function editLoad() {
		...
		if (!empty($item_id)) {
			...
			if ($this->openbay->addonLoad('openstock') && $product_info['has_option'] == 1) {
				...
				$product_info['option_grp'] = $this->model_openbay_ebay_product->getProductOptions_($this->request->get['product_id']);
	...
	public function create() {
		...
		$this->initTpl('openbay/ebay_new');
				...
				$product_info['attributes'] = base64_encode(json_encode($this->model_openbay_ebay->getProductAttributes_($this->request->get['product_id'])));
				...
				//images
				$product_images = $this->model_catalog_product->getProductImages_($this->request->get['product_id']);
				...
				$product_info['manufacturers']                      = $this->model_catalog_manufacturer->getManufacturers_();
			...
			if(!empty($this->request->post['selected'])) {
				...
				foreach ($this->request->post['selected'] as $product_id) {
					if(!array_key_exists($product_id, $activeList)) {
						...
						$prod = $this->model_catalog_product->getProduct_($product_id);
				...
				$this->initNotice();		
				$this->initTpl('openbay/ebay_new_bulk');
	...
	public function verifyBulk() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ($this->checkConfig() == true) {
				...
				$product_info               = $this->model_catalog_product->getProduct_($post['product_id']);
				...
				//product attributes - this is just a direct pass through used with the template tag
				$data['attributes'] = base64_encode(json_encode($this->model_openbay_ebay->getProductAttributes_($post['product_id'])));
				...
				$product_images = $this->model_catalog_product->getProductImages_($post['product_id']);
