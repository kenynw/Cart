...
class ControllerOpenbayAmazonusProduct extends Controller{
	private $route = 'openbay/amazonus_product';
	private function loadText($route = '') {
		if($route != ''){
			$this->loadLang ( $route );
		}else{
		$this->loadLang ( 'openbay/amazonus_product' ); }
	}
	public function index() {
		...
		$savedListingData = $this->model_openbay_amazonus->getProduct_($product_id, $variation);
		...
		$this->initTpl('openbay/amazonus_listing_advanced');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function fillDefaultValues($product_id, $fields_array, $var = '') {
		...
		$manufacturer = $this->model_catalog_manufacturer->getManufacturer_($product_info['manufacturer_id']);
		if(!empty($manufacturer)) {
			$defaults['manufacturer'] = $manufacturer['name'];
			$defaults['brand'] = $manufacturer['name'];
		}

		$productImages = $this->model_catalog_product->getProductImages_($product_id);
		$imageIndex = 1;
	...
	private function fillSavedValues($product_id, $fields_array, $var = '') {
		...
		$this->load->model('openbay/amazonus');
		$savedListing = $this->model_openbay_amazonus->getProduct_($product_id, $var);
