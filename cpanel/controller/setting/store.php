...
class ControllerSettingStore extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validateForm ()) {
			$store_id = $rs->addStore ( $this->request->post );
			$this->loadModel ( 'setting/setting' );
			$rss = new ModelSettingSetting ( $this->registry );
			$rss->editSetting ( 'config', $this->request->post, $store_id );
	...
	public function update() {
		...
		$rs = new ModelSettingStore ( $this->registry );
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validateForm ()) {
			$rs->editStore ( $this->request->post );
			$this->loadModel ( 'setting/setting' );
			$rss = new ModelSettingSetting ( $this->registry );
			$rss->editSetting ( 'config', $this->request->post, $this->request->get ['store_id'] );
			$this->session->data ['success'] = $this->language->get ( 'text_success' );
			$this->goBack ( $this->route );
		}
		$this->getForm ();
	...
	protected function getList() {
		...
		$this->initNotice ();
		$this->initTpl ( 'setting/store_list' );
		$this->children = array (
				'common/header',
				'common/footer' 
		);
	...
	public function getForm() {
		...
		$directories = glob ( DIR_APPLICATION . 'view/*', GLOB_ONLYDIR );
		...
		if (isset ( $this->request->post ['store_id'] )) {
			...
			$this->data ['store_id'] = $store_info ['store_id'];
		} else {
			$this->data ['store_id'] = '';
		}
		$this->initNotice ();
		$this->initTpl ( 'setting/store_form' );
		$this->children = array (
				'common/header',
				'common/footer' 
		);
