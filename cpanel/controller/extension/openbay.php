...
class ControllerExtensionOpenbay extends Controller {
	private $route = 'extension/openbay';
	...
	public function index() {
		...
		$this->data = array_merge($this->data, $this->load->language($this->route));
	...
	public function manage() {
		...
		$this->initTpl('openbay/openbay_manage');
		$this->children = array(
			'common/header',
			'common/footer',
		);
		...
		if ($this->config->get('openbay_status') == 1) {
			if($this->openbay->ebay->isEbayOrder($this->request->get['order_id']) !== false){
				...
				if($this->config->get('EBAY_DEF_SHIPPED_ID') == $this->request->get['status_id']){
					...
					$this->initTpl('openbay/ebay_ajax_shippinginfo');
		...
		if($this->config->get('amazon_status') == 1) {
			...
			if($this->data['order_info']){
				...
				if($this->request->get['status_id'] == $this->config->get('openbay_amazon_order_status_shipped')){
					...
					$this->initTpl('openbay/amazon_ajax_shippinginfo');
					$this->response->setOutput($this->render(true), $this->config->get('config_compression'));
				}
			}
		}
		...
		if($this->config->get('amazonus_status') == 1) {
			...
			if($this->data['order_info']){
				...
				if($this->request->get['status_id'] == $this->config->get('openbay_amazonus_order_status_shipped')){
					...
					$this->data['courier_default'] = $this->config->get('openbay_amazon_default_carrier');
					$this->initTpl('openbay/amazonus_ajax_shippinginfo');
					$this->response->setOutput($this->render(true), $this->config->get('config_compression'));
				}
			}
		}
	...
	public function orderList(){
		...
		$this->initTpl('openbay/openbay_orderlist');
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function orderListUpdate(){
		...
		if(!isset($this->request->post['selected']) || empty($this->request->post['selected'])){
			...
			$this->initTpl('openbay/openbay_orderlist_confirm');
			$this->children = array(
				'common/header',
				'common/footer'
			);
	...
	public function linkStatus(){
		...
		$this->initTpl('openbay/openbay_links');
		$this->children = array();
		$this->response->setOutput($this->render());
	}
	...
	public function itemList() {
		...
		$this->data['category_list'] = $this->model_catalog_category->getCategories_(array());
		$this->data['manufacturer_list'] = $this->model_catalog_manufacturer->getManufacturers_(array());
		...
		$product_total = $this->model_openbay_openbay->getTotalProducts_($data);

		$results = $this->model_openbay_openbay->getProducts_($data);
		...
		$this->data['order'] = $order;
		$this->initNotice();
		$this->initTpl('openbay/openbay_itemlist');
		$this->children = array(
			'common/header',
			'common/footer'
		);
