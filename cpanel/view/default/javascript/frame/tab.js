 ...
 */
 ...
 // 标签项对象 **************************
var IECTabItem = function (type,id,text,command,change)
{
	...
	// 第一个标签
	if(IECTabObject.NumberOneId == null)
		IECTabObject.NumberOneId = this.id;
	...
	if (type == 'Activate')
	{
		...
		// 第一个标签不能关掉
		if(IECTabObject.NumberOneId == this.id)
...
// 小标签对象
var IECMinItem = function (id, name, command, action)
{
	this.id = id;
	this.TagId = id;
...
// 改变标签项对象 
var IECTabItemObjectChange = function (TabItemObject, type)
{
	return new IECTabItem(type,TabItemObject.id,TabItemObject.text,TabItemObject.command,true);
}
...
var IECTab = function ()
{
	...
	{
		for (k in Item)
			this.add(new IECTabItem(Item[k].type,Item[k].id,Item[k].name, Item[k].url));
	}
	...
	this.add = function (TagItem)
	{
		...
		// 创建IECContentItem	  不管存不存在标签都做激活操作 IECContent 那边会判断
		var status = (TagItem.type == 'Activate') ? 'open':'close';
		var IECContentItem = parent.window.frames.IECContent.CreateIECContentItemObject(TagItem.id, TagItem.text, TagItem.command, status);
		parent.window.frames.IECContent.IECContentObject.AddAction(IECContentItem);
	...
	this.AddMins = function (Item)
	{
		...
		for (var k in Item)
		{
			this.ItemMin.push(new IECMinItem(Item[k][1].id, Item[k][1].name, Item[k][1].url, Item[k][1].action));
			if (Item[k][1].PlugIn)
	...
	this.TagOnclick = function (TagItem)
	{
		...
		parent.window.frames.IECContent.IECContentObject.activationAction(TagItem.id);	// 激活当前ContentItem
	...
	this.CloseTagFun = function (TagItem, RunClick)
	{
		...
		parent.window.frames.IECContent.IECContentObject.DelAction(TagItem.TagListKey, TagItem.id);	// 删除当前ContentItem
		...
		for (var i = 0; i < Item_sum; i++)
		{
			if(typeof(this.Item[i]) == 'object')
			{
				...
				if(TagItem != null && this.Item[i].id == TagItem.id)
				{
					...
					{
						var TempItemObject = IECTabItemObjectChange(TagItem, type);		// 转成type类型
				...
				else
				{
					if (type != 'Hover' || this.Item[i].type != 'Activate')
					{
						...
						{
							var TempItemObject = IECTabItemObjectChange(this.Item[i], 'Normal');		// 转成Normal类型
							TempItemObject.TagListKey = this.Item[i].TagListKey;						// 加回标签在列表的位置
							var TempItemDOM = G(this.Item[i].id);
							TempItemDOM.innerHTML = TempItemObject.I.innerHTML;
	...
	// 向应中键滑动
	this.IECMouseAction = function (e)
	{
		...
		with(this)
		{
			...
			if(Firefox.indexOf("Firefox") == -1) // IE
 				...
 				Action = -Action;
 			if(Action > 0) 
 				IECTabObject.PreAction();	
 			if(Action < 0) 
 				IECTabObject.NextAction();
 		}
	}
	...
	this.run = function ()
	{
		...
		with(this)
		{
			...
			{
				if(typeof(parent.window.frames.IECLeft.IECLNO) == 'object')
				{	
					parent.window.frames.IECLeft.IECLNO.show();
			...
			{
				ShowBlock.addEventListener('DOMMouseScroll',IECMouseAction,true); 
...
{
	var ItemMinWidth = 0;
	for (var k in IECTabObject.ItemMin )
		ItemMinWidth += IECTabObject.ItemMin[k].span.clientWidth;

	IECTabObject.ShowBlockWidth = document.body.clientWidth - G("Blank").clientWidth - ItemMinWidth - IECTabObject.New.clientWidth - 27;
	G('ShowBlock').style.cssText = 'width:' + IECTabObject.ShowBlockWidth  + 'px';
}
window.onresize = function()
{
	// copyright show
	parent.frames.IECLeft.G('copyright').style.display = parent.frames.IECLeft.getClientHeight()-70-parent.frames.IECLeft.IECLNO.LeftNavigationDom.clientHeight > parent.frames.IECLeft.G('IECLeftList').scrollHeight ? 'block':'none';
	SetShowBlockWidth();
}
...
{
	return new IECTabItem(type,id,text,command);
}
