 ...
 * @version 1.4.1
 */
class ModelInfoFilter extends Model {
	private $table = 'filter';
	private $table_group = 'filter_group';
	...
	public function getFilterGroups($data = array(), $sort_data = array()) {
	...
	public function getFilters($param = array(), $sort_data = array()) {
		...
		if (! empty ( $param ['filter_name'] )) {
			$sql .= " AND fd.name LIKE '" . $this->db->escape ( $param ['filter_name'] ) . "%'";
		}
	...
	public function getTotalFilterGroups() {
		return $this->getModelTotal ( $this->table_group );
	}
