...
class ModelCatalogProduct extends Model {
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data = array_merge ( $data, array (
					'product_filter' => $this->getProductFilters ( $product_id ) 
			) );
			$data = array_merge ( $data, array (
			...
			$data = array_merge ( $data, array (
					'product_profiles' => $this->getProfiles ( $product_id ) 
			) );
	...
	public function getProduct_($product_id) {
		$query = $this->db->query ( "SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . ( int ) $product_id . "') AS keyword FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . ( int ) $product_id . "' AND pd.language_id = '" . ( int ) $this->config->get ( 'config_language_id' ) . "'" );
		return $query->row;
	}
	public function getProducts_($data = array()) {
	...
	public function getProductDiscounts_($product_id) {
		$query = $this->db->query ( "SELECT * FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . ( int ) $product_id . "' ORDER BY quantity, priority, price" );
		return $query->rows;
	}
	public function getProductSpecials_($product_id) {
	...
	public function getProductRelated_($product_id) {
	...
	public function getDefaultProduct() {
		return $this->getModelDefault ( "`" . DB_PREFIX . "product`" );
	}
