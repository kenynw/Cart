...
class ControllerProductCategory extends Controller {
	private $route = 'product/category';
	...
	public function index() {
		...
		$category_info = $rsc->getCategory ( $category_id );
		if ($category_info) {
			...
			if ($category_info ['image']) {
				$this->data ['thumb'] = $rsi->resize ( $category_info ['image'], $this->config->get ( 'config_image_category_width' ), $this->config->get ( 'config_image_category_height' ) );
			} else {
				$this->data ['thumb'] = '';
			}
			...
			$results = $rsc->getCategories ( $category_id );
			foreach ( $results as $result ) {
				$param = array (
						'filter_category_id' => $result ['category_id'],
						'filter_sub_category' => true 
				);
				$product_total = $rsp->getTotalProducts ( $param );
				$this->data ['categories'] [] = array (
			...
			$param = array (
					...
					'sort' => $sort,
					'order' => $order,
					'start' => ($page_page - 1) * $page_limit,
					'limit' => $page_limit 
			);
			$product_total = $rsp->getTotalProducts ( $param );
			$results = $rsp->getProducts ( $param );
			foreach ( $results as $result ) {
				if ($result ['image']) {
